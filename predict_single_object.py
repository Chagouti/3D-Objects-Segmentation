...
def main(args):
    ...
    test_dataset = IntrADataLoader(root=data_path, num_point=args.num_point, num_category=args.num_category, use_uniform_sample=args.use_uniform_sample, use_normals=args.use_normals, split='test', process_data=False)
    ...
    if args.use_cpu:
        ...
        checkpoint = torch.load('/content/IntrA-3D-Objects-Classification/log/classification/pointnet2_msg_normals/checkpoints/best_model.pth', map_location=torch.device('cpu'))  # Load checkpoint on CPU
    else:
        classifier = classifier.cuda()
        checkpoint = torch.load('/content/IntrA-3D-Objects-Classification/log/classification/pointnet2_msg_normals/checkpoints/best_model.pth')
